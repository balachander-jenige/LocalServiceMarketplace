...
from ..dto.auth_dto import LoginRequest, RegisterRequest, RegisterResponse, TokenResponse
...
async def register_admin(data: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if data.role_id != 3:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Admin registration requires role_id=3")

    user = await AuthService.register(
        db=db, username=data.username, email=data.email, password=data.password, role_id=3  # Admin role
    )
    return RegisterResponse(id=user.id, username=user.username, email=user.email)
