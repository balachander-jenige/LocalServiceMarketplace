...
class ProviderProfileService:
    ...
    async def create_profile(
        ...
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Provider profile already exists")
        ...
        # 发布事件
        event = ProfileCreatedEvent(user_id=user_id, profile_type="provider", timestamp=datetime.utcnow())
    ...
    async def update_profile(self, user_id: int, update_data: dict):
        ...
        event = ProfileUpdatedEvent(
            user_id=user_id, profile_type="provider", updated_fields=update_data, timestamp=datetime.utcnow()
        )
