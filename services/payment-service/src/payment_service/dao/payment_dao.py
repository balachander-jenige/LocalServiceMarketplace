...
class PaymentDAO:
    ...
    async def get_payment_by_order_id(db: AsyncSession, order_id: int) -> Optional[Payment]:
        """ByOrder ID GetPayment记录"""
        result = await db.execute(select(Payment).where(Payment.order_id == order_id))
        return result.scalars().first()

    @staticmethod
    async def update_payment_status(db: AsyncSession, payment_id: int, status: PaymentStatus) -> Optional[Payment]:
        """UpdatePaymentStatus"""
    ...
    async def get_user_payments(db: AsyncSession, user_id: int) -> List[Payment]:
        ...
        result = await db.execute(
            select(Payment).where(Payment.customer_id == user_id).order_by(Payment.created_at.desc())
        )
