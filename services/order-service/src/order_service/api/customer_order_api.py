...
from ..dto.order_dto import CancelOrderResponse, OrderDetail, OrderSummary, PublishOrderRequest, PublishOrderResponse
...
async def publish_order(
    ...
    return PublishOrderResponse(
        order_id=order.id, status=order.status.value, message=f"You have successfully published the order: {order.id}."
    )
...
async def cancel_order(order_id: int, user_id: int = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
    ...
    return CancelOrderResponse(
        order_id=order.id, status=order.status.value, message=f"You have successfully cancelled the order: {order.id}."
    )
